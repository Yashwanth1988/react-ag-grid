    ...
    const dataSource = {
        ...
        getRows: (params) => {
            ...
            if (count.current == 0) {
                ...
                const filterComponent = paramsRef.current.parentNode.gridApi.getFilterInstance("employee");
                filterComponent.setModel({
                    type: "startsWith",
                    filter: filterText
                });
                paramsRef.current.parentNode.gridApi.onFilterChanged()
                count.current = 1;
                return
            } else {
                if (count.current == 1) {
        ...
        if (paramsRef.current) {
            ...
            Axios.get(apiURL).then((records) => {
                ...
                if (records.data.length != 0) {
                    paramsRef.current.parentNode.gridApi.hideOverlay()
                } else {
                    paramsRef.current.parentNode.gridApi.showNoRowsOverlay()
                }
            })
